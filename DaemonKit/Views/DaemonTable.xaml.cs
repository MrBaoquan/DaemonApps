        ...
        public DaemonTable()
        {
            ...
            this.WhenActivated(
                (disposables) =>
                {
                    ...
                    {
                        var _commandBuffer = Encoding.UTF8.GetBytes(
                            JsonConvert.SerializeObject(new Command { EventID = Command.RESTART })
                        );
                        _commandClient.Send(
                            _commandBuffer,
                            _commandBuffer.Length,
                            new IPEndPoint(IPAddress.Parse(_machineInfo.ID), CommonVars.ControlPort)
                        );
                    });
                    ...
                    {
                        var _commandBuffer = Encoding.UTF8.GetBytes(
                            JsonConvert.SerializeObject(new Command { EventID = Command.SHUTDOWN })
                        );
                        _commandClient.Send(
                            _commandBuffer,
                            _commandBuffer.Length,
                    ...
                    ViewModel.TryRestartNodeTree.Subscribe(_machineInfo =>
                    {
                        var _commandBuffer = Encoding.UTF8.GetBytes(
                            JsonConvert.SerializeObject(
                                new Command { EventID = Command.RESTART_NODE_TREE }
                            )
                        );
                        _commandClient.Send(
                            _commandBuffer,
